...
<body class="bg-gray-100 p-4 lg:p-8">
    <div id="app" class="max-w-7xl mx-auto">
        <!-- Header -->
        <div class="bg-white rounded-xl shadow-lg p-6 mb-8 flex flex-col md:flex-row justify-between items-center gap-4 no-print">
            <div>
            ...
            <div class="flex flex-col gap-4 w-full md:w-auto">
                <select v-model="selectedClient" class="w-full md:w-64 p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-lg font-bold">
                    <option value="">-- اختر العميل --</option>
                    <option v-for="client in clients" :value="client.name">{{ client.name }}</option>
                </select>
                <button @click="printPage" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-bold transition flex items-center justify-center gap-2">
        ...
        <div class="hidden print:block mb-8 text-center border-b-2 border-gray-800 pb-4">
            <h1 class="text-3xl font-bold text-gray-800">كشف مبيعات وتوريدات الأصناف</h1>
            <h2 class="text-2xl font-bold mt-2" v-if="activeClient">العميل: {{ activeClient.name }}</h2>
        </div>
        ...
        <div v-if="activeClient" class="space-y-8">
            ...
            <!-- Items Loop -->
            <div v-for="item in activeClient.items" :key="item.name" class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200 print:border-gray-800 print:shadow-none mb-6 print:break-inside-avoid">
                <div class="bg-gray-50 border-b border-gray-200 p-4 print:bg-gray-100">
                    <h2 class="text-xl font-bold text-gray-800"><i class="fa-solid fa-box text-orange-500 ml-2 no-print"></i> الصنف: {{ item.name }}</h2>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-right text-sm">
                        <thead class="table-header print:bg-gray-200 print:text-black">
                            <tr>
                                <th class="p-3">التاريخ</th>
                                <th class="p-3">رقم الفاتورة</th>
                                <th class="p-3">النوع</th>
                                <th class="p-3 text-center">الكمية الأصلية</th>
                                <th class="p-3 text-center">الكمية الصافية</th>
                        ...
                        <tbody>
                            <tr v-for="t in item.transactions" class="border-b row-hover" :class="{'bg-red-50': t.qty < 0, 'bg-orange-50': t.status === 'partially_returned'}">
                                ...
                                <td class="p-3">
                                    <span v-if="t.qty < 0" class="text-red-600 font-bold"><i class="fa-solid fa-arrow-right-arrow-left no-print"></i> {{ t.type }}</span>
                                    <span v-else class="text-green-600 font-bold"><i class="fa-solid fa-check no-print"></i> {{ t.type }}</span>
                                </td>
                                <td class="p-3 text-center text-gray-400 line-through" v-if="t.qty > 0 && t.originalQty !== t.qty">{{ t.originalQty }}</td>
                                <td class="p-3 text-center text-gray-400" v-else>
                                    <span v-if="t.qty > 0">{{ t.originalQty }}</span>
                                    <span v-else>-</span>
                                </td>
                                <td class="p-3 font-bold text-lg text-center" :class="t.qty < 0 ? 'text-red-600' : 'text-gray-800'" dir="ltr">{{ Math.abs(t.qty) }}</td>
                                <td class="p-3">{{ t.price.toLocaleString() }}</td>
                                <td class="p-3 font-bold" :class="t.total < 0 ? 'text-red-600' : 'text-blue-600'" dir="ltr">{{ Math.abs(t.total).toLocaleString() }}</td>
                            </tr>
                        </tbody>
                        <tfoot class="bg-gray-100 font-bold text-gray-800 border-t-2 border-gray-300 print:bg-gray-200">
                            <tr>
                                <td colspan="4" class="p-4 text-left text-lg">إجمالي حركة الصنف:</td>
                                <td class="p-4 text-xl text-blue-800 text-center" dir="ltr">{{ item.totalQty }}</td>
                                <td></td>
                                <td class="p-4 text-xl text-blue-800" dir="ltr">{{ item.totalAmt.toLocaleString() }} ج.م</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
            ...
            <!-- Grand Totals -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 print:mt-12 print:break-inside-avoid">
                <div class="bg-white rounded-xl shadow p-6 border-r-4 border-blue-500 print:shadow-none print:border-2">
                ...
                <div class="bg-white rounded-xl shadow p-6 border-r-4 border-red-500 print:shadow-none print:border-2">
                ...
                <div class="bg-white rounded-xl shadow p-6 border-r-4 border-green-500 print:shadow-none print:border-2">
                    <p class="text-gray-500 text-sm font-bold mb-1">الصافي النهائي للعميل</p>
        ...
        <!-- Empty State -->
        <div v-else class="bg-white rounded-xl shadow p-12 text-center no-print">
...
fs.writeFileSync('build_dashboard.js', base + newLogic + htmlTemplate, 'utf8');
